...
export function useOpenCode(options: UseOpenCodeOptions = {}): UseOpenCodeReturn {
  ...
  const syncFromStore = useCallback(() => {
    ...
    if (currentSessionId) {
      ...
      // Collect all parts for current session
  ...
  }, [baseUrl, directory, autoConnect, handleEvent])
  ...
  const getPartsForMessage = useCallback((messageId: string): Part[] => {
