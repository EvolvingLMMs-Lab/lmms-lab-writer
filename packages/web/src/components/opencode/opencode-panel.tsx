...
import type { Message, Part, ToolPart, TextPart, SessionStatus } from '@/lib/opencode/types'
...
export function OpenCodePanel({ className = '', baseUrl, directory }: Props) {
  const opencode = useOpenCode({ baseUrl, directory, autoConnect: false })
  ...
  const isWorking = opencode.status.type === 'running' || opencode.status.type === 'retry'
  ...
  return (
    <div className="h-full flex flex-col items-center justify-center text-center space-y-4">
      ...
      <div className="text-xs text-muted space-y-1">
        ...
        <code className="bg-accent px-2 py-1 block">opencode web --port 4096</code>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="h-full flex flex-col items-center justify-center text-center space-y-4">
      <div className="text-muted">
        ...
        <p className="text-xs mt-2">Create a new session to start chatting with OpenCode.</p>
      </div>
      <button
...
function MessageList({ 
  ...
  let currentTurn: { user: Message; assistant: Message[]; parts: Part[] } | null = null

  for (const msg of messages) {
    if (msg.role === 'user') {
      if (currentTurn) turns.push(currentTurn)
      currentTurn = { user: msg, assistant: [], parts: getPartsForMessage(msg.id) }
...
function MessageTurn({ 
  ...
  const userText = userParts.find(p => p.type === 'text') as TextPart | undefined
  ...
  const isWorking = isLast && (status.type === 'running' || status.type === 'retry')
    ...
    <div className="space-y-3">
      ...
      <div className="flex gap-2">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm whitespace-pre-wrap break-words">{userText?.text || ''}</p>
        </div>
      </div>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        ...
        {isComplete && (
          <svg className="size-3 text-black" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {isComplete && (part.tool === 'edit' || part.tool === 'write') && (
          <DiffStats metadata={(part.state as { metadata?: Record<string, unknown> }).metadata} />
        )}
      </div>
      
      {isError && (
        <p className="mt-1 text-red-600">{(part.state as { error: string }).error}</p>
      )}
    </div>
  )
}
      ...
      {parts.map((part, i) => {
        if (part.startsWith('```')) {
          ...
          if (match) {
            const [, , code] = match
            return (
              <pre key={i} className="bg-accent p-2 overflow-x-auto text-xs my-2">
                <code>{code?.trim()}</code>
              </pre>
            )
          }
          ...
          <span key={i}>
            {inlineParts.map((p, j) => {
              if (p.startsWith('`') && p.endsWith('`')) {
                return <code key={j} className="bg-accent px-1 text-xs">{p.slice(1, -1)}</code>
              }
              return p
            })}
          </span>
