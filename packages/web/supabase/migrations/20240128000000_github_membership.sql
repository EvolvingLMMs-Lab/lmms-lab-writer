...
create index idx_user_github_tokens_github_id on user_github_tokens(github_id);
create index idx_user_github_tokens_username on user_github_tokens(github_username);
...
-- Max 4 stars = max 30 days (7 days per star, capped at 30)
...
begin
  if star_count >= 1 then
    effective_stars := least(star_count, 4);
    days_granted := least(effective_stars * 7, 30);
...
-- Update user_storage to integrate with membership tiers
alter table user_storage add column if not exists tier_max_bytes bigint;

create or replace function get_storage_limit_for_tier(tier text)
returns bigint as $$
begin
  case tier
