...
cli
  ...
  .option(
    "-t, --template <template>",
    "Project template (article, thesis, beamer, neurips, iclr, tech-report)",
    { default: "article" },
  )
...
cli.command("login", "Login to LaTeX Writer").action(async () => {
  await login();
});

cli.command("logout", "Logout from LaTeX Writer").action(async () => {
  await logout();
});

cli.command("whoami", "Show current logged in user").action(async () => {
  await whoami();
});
